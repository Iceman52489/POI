...
<cfswitch expression="#THISTAG.ExecutionMode#">
	<cfcase value="Start">
		...
		--->
		<cfparam name="ATTRIBUTES.FreezeRow" type="string" default="0" />
		<!---
		...
		--->
		<cfparam name="ATTRIBUTES.FreezeColumn" type="string" default="0" />
		<!---
			The print orientation of the sheet. Defaults to portrait. Can also
			be "landscape" orientation.
		--->
		<cfparam name="ATTRIBUTES.Orientation" type="string" default="portrait" />
		<!---
			The default zoom of the sheet. This is entered as a percentage.
			100% would be the normal zoom.
		--->
		<cfparam name="ATTRIBUTES.Zoom" type="regex" pattern="^[\d]+%$" default="100%" />
		...
		<cfelse>
			...
			<cfset VARIABLES.Sheet = VARIABLES.DocumentTag.WorkBook.CreateSheet( JavaCast( "string", ATTRIBUTES.Name ) ) />
		</cfif>
		...
		<cfset VARIABLES.Scale = Val( ATTRIBUTES.Scale ) />
		<!---
	...
	<cfcase value="End">
		...
		--->
		<cfloop item="VARIABLES.ColumnIndex" collection="#VARIABLES.ColumnClasses#">
		...
		<cfscript>
			// Let's check to see if we have any freeze panes to create based on our frozen row and column.
			if(VARIABLES.FreezeRow GT 0 OR VARIABLES.FreezeColumn GT 0) {
				// Create a freeze pane.
				VARIABLES.Sheet.CreateFreezePane(JavaCast("int", VARIABLES.FreezeColumn), JavaCast("int", VARIABLES.FreezeRow));
			}

			// Check to see if the print orientation was set to landscape.
			if(ATTRIBUTES.Orientation EQ "landscape") {
				// Get the print setup and set the landscape orientation of the current sheet.
				VARIABLES.Sheet.GetPrintSetup().SetLandscape(JavaCast("boolean", true));
			}

			// Set the sheet zoom.
			VARIABLES.Sheet.SetZoom(JavaCast("int", VARIABLES.Zoom), JavaCast("int", 100));

			// Set the sheet zoom.
			VARIABLES.Sheet.GetPrintSetup().setScale(JavaCast("int", VARIABLES.Scale));

			// Set Fit to Page
			VARIABLES.Sheet.setAutobreaks(JavaCast("boolean", true));

			VARIABLES.Sheet.GetPrintSetup().setFitHeight(JavaCast("short", 1));
			VARIABLES.Sheet.GetPrintSetup().setFitWidth(JavaCast("short", 1));

			// Update the sheet count.
			VARIABLES.DocumentTag.SheetIndex++;
		</cfscript>
	</cfcase>
</cfswitch>
