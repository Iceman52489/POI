...
<cfcomponent
	...
	<cffunction name="Init" access="public" returntype="POIUtility" output="false"
		...
		<cfset THIS.JavaLoader = SERVER[APPLICATION.javaLoaderKey]>
	...
	<cffunction name="ReadExcel" access="public" returntype="any" output="true"
		...
		<cfscript>
			...
			LOCAL.ExcelFileSystem = THIS.JavaLoader.Create("org.apache.poi.poifs.filesystem.POIFSFileSystem").Init(LOCAL.FileInputStream);
			LOCAL.WorkBook = THIS.JavaLoader.Create("org.apache.poi.ss.extractor.ExcelExtractor");//.Init(LOCAL.ExcelFileSystem);
			writeDump(local);abort;

			// Get the workbook from the Excel file system.
/*			LOCAL.WorkBook = THIS.JavaLoader.Create(
...
*/		
	...
	<cffunction name="ReadExcelSheet" access="public" returntype="struct" output="true"
